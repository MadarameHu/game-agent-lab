...
# Track-tuned controls. Steering rates are radians/second, response is 1/seconds.
@export_group("Driving")
@export_range(10.0, 250.0, 1.0) var drive_force: float = 100.0
@export_range(0.2, 6.0, 0.05) var low_speed_steering: float = 3.4
@export_range(0.2, 3.0, 0.05) var high_speed_steering: float = 0.95
@export_range(0.5, 15.0, 0.1) var steering_response: float = 6.0
@export_range(0.2, 8.0, 0.1) var coast_response: float = 2.0

func steering_gain(speed_mps: float) -> float:
	return lerpf(low_speed_steering, high_speed_steering, smoothstep(2.0, 8.0, speed_mps))
...
func _physics_process(delta):
	...
	var horizontal_speed = Vector2(sphere.linear_velocity.x, sphere.linear_velocity.z).length()
	var target_angular = -input.x * steering_grip * steering_gain(horizontal_speed) * direction
	angular_speed = lerpf(angular_speed, target_angular, 1.0 - exp(-steering_response * delta))
	...
	if (target_speed < 0 and linear_speed > 0.01):
		...
		if (target_speed < 0):
			...
			linear_speed = lerpf(linear_speed, target_speed, 1.0 - exp(-(coast_response if is_zero_approx(target_speed) else 6.0) * delta))
...
func handle_input(delta):
	...
	sphere.angular_velocity += vehicle_model.get_global_transform().basis.x * (linear_speed * drive_force) * delta
