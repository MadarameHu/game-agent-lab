...
func test():
    ...
    main.get_node("HUD").restart_button.pressed.emit()
    ...
    check("restart_restores_world",restored,{"coins":coins,"fixture":"Triggered actual product Button.pressed signal; no manual world reset","old_scene_id":old_id,"new_scene_id":restarted.get_instance_id()})
