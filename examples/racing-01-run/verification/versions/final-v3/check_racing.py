...
def main():
 ...
 checks={'project_loads':load_good,'protected_gameplay':not differences and main_unchanged,'closed_track':report.get('closed_track',False),'required_geometry':report.get('required_geometry',False),'spawn_and_decor':report.get('spawn_and_decor',False),'physical_lap':bool(lap and lap.get('passed') and lap.get('physics_hz')==60 and commands[-1]['exit_code']==0 and not commands[-1]['timed_out'])}
 result={'checks':checks,'engine_reward':int(all(checks.values())),'human_review':None,'combined_reward':None,'resource_or_script_errors':error_lines,'protected_file_differences':differences,'main_outside_gridmap_unchanged':main_unchanged,'physical_lap_status':lap.get('status') if lap else 'not_run','source_main_sha256':hashlib.sha256((a.project/'scenes/main.tscn').read_bytes()).hexdigest(),'driver_sha256':hashlib.sha256((HERE/'drive_lap.gd').read_bytes()).hexdigest(),'checker_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'contract_sha256':hashlib.sha256((HERE.parent/'contract/acceptance.json').read_bytes()).hexdigest(),'snapshot':str(snapshot),'commands':commands,'note':'Failure of one bounded input-only test driver is not proof that a human cannot drive the course.'}
