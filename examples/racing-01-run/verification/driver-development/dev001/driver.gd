...
func finish(reason,passed):
 ...
 var result={"status":reason,"passed":passed,"driver":"input-pure-pursuit-v1","physics_steps":step,"physics_hz":Engine.physics_ticks_per_second,"simulation_seconds":float(step)/60.0,"wall_seconds":float(Time.get_ticks_usec()-started_usec)/1000000.0,"waypoints_total":route.size(),"waypoints_completed":next_index,"physical_start":vec(start_position),"physical_end":vec(sphere.global_position),"return_distance_xz":xz(sphere.global_position).distance_to(xz(start_position)),"distance_traveled_m":traveled,"max_local_centerline_distance_m":max_deviation,"max_sphere_y":max_y,"reverse_action_used":false,"parameters":{"lookahead_m":lookahead,"forward_strength":throttle,"waypoint_radius_m":waypoint_radius,"settle_steps_included":SETTLE_STEPS},"criteria":{"max_steps":MAX_STEPS,"max_centerline_distance_m":MAX_DEVIATION,"return_radius_m":RETURN_RADIUS},"note":"Only Input.action_press/release used. Stock controller and rigid body remain unmodified. A driver failure alone does not prove human undrivability."}
...
func _physics_process(_delta):
 ...
 telemetry.store_line(JSON.stringify({"step":step,"position":vec(position),"velocity":vec(velocity),"speed":xz(velocity).length(),"heading":vec(heading),"ground":ground,"next_waypoint":next_index,"local_centerline_distance":local_distance,"traveled":traveled,"input":{"forward":throttle if step>SETTLE_STEPS else 0.0,"left":maxf(-steer,0.0),"right":maxf(steer,0.0),"back":0.0}}))
 ...
 if ground and xz(position).distance_to(xz(route[next_index]))<=waypoint_radius:
  next_index+=1
